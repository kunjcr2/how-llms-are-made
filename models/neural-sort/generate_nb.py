...
text_intro = """## 1. What is a Pointer Network?
A Pointer Network is a neural architecture that learns the conditional probability of an output sequence with elements that are discrete tokens corresponding to positions in an input sequence. Instead of generating new outputs from a fixed vocabulary, it uses attention mechanisms as a "pointer" to select members of the input sequence as output. This makes it perfect for tasks like sorting, where the output is simply a permutation of the input elements."""

code_setup = """!pip install torch numpy"""

text_data = """## 2. Data generation"""
code_data = """import torch
...
text_model = """## 3. Model definition"""
code_model = """import torch.nn as nn
import math
...
text_train = """## 4. Training loop"""
code_train = """import torch.optim as optim
...
text_eval = """## 5. Evaluation"""
code_eval = """def num_inversions(arr):
    inv = 0
    size = len(arr)
    for i in range(size):
...
text_demo = """## 6. Demo"""
code_demo = """# No matplotlib required for simple array printing constraints
...
nb['cells'] = [
    ...
    nbf.v4.new_markdown_cell(text_demo),
    nbf.v4.new_code_cell(code_demo),
]
